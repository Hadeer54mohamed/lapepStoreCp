...
export default function EditBannerPage() {
  ...
  <header className="flex justify-between items-center">
    <section className="text-right">
      <h1 className="text-2xl font-bold text-[#011957] dark:text-white">
        تعديل البانر
      </h1>
      <p className="text-gray-600 dark:text-gray-300">تعديل بيانات البانر</p>
    </section>
    <Link
      href="/dashboard/banners"
      className="bg-gradient-to-r from-[#6043FD] to-[#9861FB] hover:from-[#5033e0] hover:to-[#8750e0] text-white px-4 py-2 rounded-lg transition flex items-center gap-2 shadow"
    >
  ...
  {/* Form Section */}
  <section className="bg-white dark:bg-[#0c1427] rounded-xl shadow-lg border border-[#BA6FEE]">
    <div className="p-6">
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Image Upload */}
        <div>
          <label className="block text-sm font-medium text-[#011957] dark:text-gray-300 mb-2">
            صورة البانر
          </label>
          <div className="space-y-4">
            <input
              ...
              className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-[#F3EBFF] file:text-[#6043FD] hover:file:bg-[#E0D4FF] dark:file:bg-[#1e1a3c] dark:file:text-white"
            />
            ...
            {imagePreview && (
              <div className="mt-4">
                ...
                <Image
                  ...
                  width={400}
                  height={192}
                  className="w-full max-w-md h-48 object-cover rounded-lg border border-[#BA6FEE]"
                />
              </div>
            )}
      ...
      {/* Arabic Description */}
<div className="mt-6">
  <label className="block text-sm font-medium text-[#011957] dark:text-white mb-2">
    وصف البانر (بالعربي)
  </label>
  <EditorProvider>
    <Toolbar>
    ...
    />
        ...
        <div className="flex gap-4 pt-6">
          <button
            type="submit"
            disabled={
              updateBannerMutation.isPending || uploadImageMutation.isPending
            }
            className="bg-gradient-to-r from-[#6043FD] to-[#9861FB] hover:from-[#5033e0] hover:to-[#8750e0] text-white px-6 py-2 rounded-lg transition flex items-center gap-2 shadow disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {updateBannerMutation.isPending || uploadImageMutation.isPending ? (
              <>
          ...
          <Link
            href="/dashboard/banners"
            className="bg-[#E10E0E] hover:bg-red-600 text-white px-6 py-2 rounded-lg transition flex items-center gap-2 shadow"
          >
